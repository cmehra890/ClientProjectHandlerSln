        ...
        public async Task<IActionResult> Index(LoginModel? model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        _contextAccessor?.HttpContext?.Session.SetString(SessionVariablesModel.SessionUserId, response?.RoleId);
                        _contextAccessor?.HttpContext?.Session.SetString(SessionVariablesModel.SessionSystemRole, response?.SystemRole);
